...
const Calendrier = (() => {
  ...
  async function render() {
      ...
      <div class="card mt-3">
        <div class="card-header" style="justify-content:space-between;align-items:flex-start;">
          <div style="display:flex;gap:12px;align-items:center;">
            ...
            <div>
              <div class="card-title">Exporter vers Google Calendar</div>
              <div class="card-subtitle">Rappels vaccins, chaleurs, naissances — fichier .ics universel</div>
            </div>
          </div>
        ...
        <button class="btn btn-primary btn-block" id="export-ics-btn">
          ${Icons.get('download', 16)} Télécharger le fichier .ics
        </button>
    ...
    document.getElementById('ics-help-btn').addEventListener('click', () => UI.showHelp(
      'Export agenda .ics',
      'Le fichier .ics (iCalendar) est un format universel d\'agenda.<br><br>' +
      '<strong>Comment l\'importer :</strong><br>' +
      '• <strong>Google Calendar</strong> : Paramètres → Importer → ouvrir le fichier<br>' +
      '• <strong>Apple Calendar (iPhone/Mac)</strong> : ouvrir le fichier .ics → "Importer"<br>' +
      '• <strong>Outlook</strong> : Fichier → Ouvrir et exporter → Importer<br><br>' +
      'Le fichier contient vos rappels santé, les prochaines chaleurs prédites et les dates de mise bas estimées.'
    ));
    ...
    try {
      ...
      for (const key of sortedKeys) {
        ...
        for (const evt of dayEvents) {
          ...
          const clickAttr = evt.animalId ? `data-nav="animal-detail" data-params='{"id":"${evt.animalId}"}'` : '';
