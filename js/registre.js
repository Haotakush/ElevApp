      ...
      document.getElementById('registre-content').innerHTML = `
        ${alertes72h.length > 0 ? `
          <div class="alert-card mb-2" style="border-left: 3px solid var(--red);">
            <div class="alert-icon">${Icons.get('alertTriangle')}</div>
        ...
        <div class="card mb-2">
          <button class="btn btn-primary btn-block" id="btn-nouvelle-entree">
            ${Icons.get('plus', 14)} Nouvelle entrée d'animal
          </button>
        ...
        <div class="section-title"><div class="section-icon">${Icons.get('health', 18)}</div> Historique des mouvements</div>
            ...
            return `
              <div class="card mb-1">
                <div style="display:flex;align-items:center;gap:10px;">
                  <svg width="16" height="16"><circle cx="8" cy="8" r="8" fill="${isEntree ? 'var(--green)' : 'var(--red)'}"/></svg>
                  <div style="flex:1;">
  ...
  async function renderFormMouvement(direction) {
      ...
      <form id="mouvement-form">
        <div class="card mb-2">
          ...
          <div class="form-group">
            ...
            <input class="form-input" type="text" inputmode="numeric" placeholder="JJ/MM/AAAA" data-date-input="1" id="mvt-date" required value="${Utils.todayInput()}">
          </div>
        ...
        <button type="submit" class="btn btn-primary btn-block mt-2 mb-3">
          ${direction === 'entree' ? `${Icons.get('plus', 14)} Enregistrer l'entrée` : `— Enregistrer la sortie`}
        </button>
      </form>
    `);
    ...
    document.getElementById('mouvement-form').addEventListener('submit', async (e) => {
      ...
      const mouvementData = {
        ...
        date: (() => { const d = Utils.parseDateInput(document.getElementById('mvt-date').value); return d ? d.toISOString().split('T')[0] : ''; })(),
        motif: motif,
