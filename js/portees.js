...
const Portees = (() => {
  ...
  async function loadPorteesList() {
    ...
    try {
      let portees = await getPortees(uid, currentFilter || undefined);
      ...
      // Fetch animal names for mere and pere
      const animals = await DB.getAnimals(uid);
      const externes = typeof Externes !== 'undefined' ? await Externes.getExternes(uid) : [];

      listEl.innerHTML = portees.map(portee => renderPorteeCard(portee, animals, externes)).join('');
  ...
  function renderPorteeCard(portee, animals, externes) {
    ...
    const pereName = pere ? Utils.escapeHtml(pere.nom) : '—';
  ...
  async function renderForm(porteeId) {
    ...
    const selectedPereType = portee?.pereExterneId ? 'externe' : 'interne';
    ...
    let html = `
      ...
      <form id="portee-form">
        ...
        <div class="card mb-2">
          ...
          <div class="form-group" id="pere-externe-group" style="display:${selectedPereType === 'externe' ? 'block' : 'none'};">
            <label class="form-label" for="portee-pere-externe">Père externe <span class="required">*</span></label>
            <select class="form-select" id="portee-pere-externe">
              <option value="">Choisir un père externe...</option>
              ${externes.map(e => `
                <option value="${e.id}" ${portee?.pereExterneId === e.id ? 'selected' : ''}>
                  ${Utils.getEspeceEmoji(e.espece || 'canin')} ${Utils.escapeHtml(e.nom)}
                </option>
              `).join('')}
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <input class="form-input" type="date" id="portee-date-prevue" required value="${portee?.datePrevue ? Utils.formatDateISO(portee.datePrevue) : ''}" readonly style="background:var(--bg-light);cursor:default;">
            <div class="form-hint" id="date-prevue-hint"></div>
          </div>
        </div>
        ...
        <div class="card mb-2">
          <div class="form-group">
            <label class="form-label" for="portee-date-naissance">Date de naissance effective</label>
    ...
    // ---- Event: Date accouplement -> recalc date prévue ----
    document.getElementById('portee-date-accouplement').addEventListener('change', () => {
      const dateAccoupStr = document.getElementById('portee-date-accouplement').value;
      if (dateAccoupStr) {
        const dateAccoup = new Date(dateAccoupStr);
        const datePrevue = calculerDatePrevue(dateAccoup, espece);
        const dateStr = Utils.formatDateISO(datePrevue);
        document.getElementById('portee-date-prevue').value = dateStr;
        document.getElementById('date-prevue-hint').textContent = `Naissance prévue le ${Utils.formatDate(datePrevue)} (${espece === 'canin' ? '63 jours' : '65 jours'})`;
      }
    });
    ...
    if (portee?.dateAccouplement) {
      ...
      document.getElementById('portee-date-accouplement').dispatchEvent(ev);
    }
    ...
    document.getElementById('portee-form').addEventListener('submit', async (e) => {
      ...
      const pereExterneId = pereType === 'externe' ? document.getElementById('portee-pere-externe').value : null;

      if (!pereId && !pereExterneId) {
        UI.toast('Veuillez sélectionner un père', 'error');
        return;
      }
      ...
      // Fallback : recalculer si le champ readonly n'a pas été rempli (bug mobile)
      if (!datePrevueStr && dateAccoupStr) {
        const dateAccoupFallback = new Date(dateAccoupStr);
        dateAccoupFallback.setDate(dateAccoupFallback.getDate() + (espece === 'canin' ? 63 : 65));
      ...
      const data = {
        ...
        statut: document.getElementById('portee-statut')?.value || 'gestation',
  ...
  async function renderDetail(porteeId) {
    ...
    let html = `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
        ...
        ` : '<div class="card" style="background:var(--bg-light);"><div style="font-size:0.8rem;color:var(--text-muted);">Père</div><div style="font-weight:600;margin-top:4px;color:var(--text-muted);">Externe</div></div>'}
      </div>
  ...
  function calculerDatePrevue(dateAccouplement, espece) {
    ...
    const jours = espece === 'canin' ? 63 : 65;
  ...
  function renderPoidsSuivi(portee) {
    ...
    dates.forEach((date, i) => {
      ...
      tableHtml += `<tr style="background:${i % 2 === 0 ? 'var(--bg-secondary)' : 'transparent'};">
        ...
        ${chiots.map(c => {
          ...
          const diff = prevPesee ? pesee.poids - prevPesee.poids : null;
