...
const Portees = (() => {
  ...
  async function renderForm(porteeId) {
    ...
    // Filter females for mere selection (actives, non stérilisées)
    const STATUTS_STERILISES = ['steriliseChirurgical', 'steriliseChimique'];
    const especeProfil = typeof Auth !== 'undefined' ? Auth.getEspeceElevee() : 'les_deux';
    const femelles = animals.filter(a =>
      a.sexe === 'femelle' &&
      a.statut === 'actif' &&
      !STATUTS_STERILISES.includes(a.statutReproducteur) &&
      (especeProfil === 'les_deux' || a.espece === especeProfil)
    );

    // Filter males for pere selection (actifs, non stérilisés, même espèce que la mère)
    const allMales = animals.filter(a =>
      a.sexe === 'male' &&
      a.statut === 'actif' &&
      !STATUTS_STERILISES.includes(a.statutReproducteur)
    );
    const males = allMales.filter(a => !selectedMereObj || a.espece === selectedMereObj.espece);
