...
const Portees = (() => {
  ...
  async function renderList() {
    UI.setContent(`
      <div class="section-title"><div class="section-icon">${Icons.get('litter', 18)}</div> Portées & Mariages</div>
      ...
      if (filteredPortees.length === 0) {
        listEl.innerHTML = `
          <div class="empty-state">
            <div class="empty-icon">${Icons.get('litter', 36)}</div>
  ...
  function renderPorteeCard(portee, animals, externes) {
    ...
    if (portee.statut === 'gestation' && portee.datePrevue) {
      ...
      additionalInfo = `<div class="card-meta" style="margin-top:8px;display:flex;align-items:center;gap:5px;">${Icons.get('calendar', 13)} Naissance prévue le ${Utils.formatDate(portee.datePrevue)} (${countdown} j.)</div>`;
      ...
      additionalInfo = `<div class="card-meta" style="margin-top:8px;display:flex;align-items:center;gap:5px;">${Icons.get('heart', 13)} ${nbChiots} ${nbChiots > 1 ? petits : petit} nés le ${Utils.formatDate(portee.dateNaissance)}</div>`;
    }
  ...
  async function renderForm(porteeId) {
          ...
          <div class="form-group">
            ...
            <div class="radio-group" style="display:flex;gap:10px;margin-bottom:8px;">
              <label style="display:flex;align-items:center;gap:4px;">
                <input type="radio" name="pere-type" value="interne" ${selectedPereType === 'interne' ? 'checked' : ''}>
                Interne
              </label>
              <label style="display:flex;align-items:center;gap:4px;">
                <input type="radio" name="pere-type" value="externe" ${selectedPereType === 'externe' ? 'checked' : ''}>
                Externe
              </label>
            </div>
          </div>
    ...
    // ---- Event: Toggle père interne/externe ----
    document.querySelectorAll('input[name="pere-type"]').forEach(radio => {
      radio.addEventListener('change', (e) => {
        const val = e.target.value;
        document.getElementById('pere-interne-group').style.display = val === 'interne' ? 'block' : 'none';
        document.getElementById('pere-externe-group').style.display = val === 'externe' ? 'block' : 'none';
        if (val === 'interne') document.getElementById('portee-pere-nom-libre').value = '';
        if (val === 'externe') document.getElementById('portee-pere').value = '';
      });
    });
  ...
  async function renderDetail(porteeId) {
    ...
    // ---- Event: Édition et export par chiot (délégation) ----
    document.getElementById('chiots-detail-list')?.addEventListener('click', (e) => {
      const editBtn = e.target.closest('[data-action="edit-chiot"]');
      if (editBtn) {
        openChiotEditInline(portee, uid, editBtn.dataset.chiotId, espece);
        return;
      }
      const exportBtn = e.target.closest('[data-action="export-chiot-pdf"]');
      if (exportBtn) {
        const chiot = (portee.chiots || []).find(c => c.id === exportBtn.dataset.chiotId);
        if (chiot) exportChiotFichePDF(portee, chiot, mereName, pereName, espece);
        return;
      }
    });
  ...
  function renderChiotDetail(chiot, porteeId) {
    ...
    return `
      <div class="card mb-2" id="chiot-card-${chiot.id}" style="border-left:4px solid var(--accent);">
        <div style="display:flex;justify-content:space-between;align-items:start;gap:8px;">
          <div style="flex:1;">
            <div class="animal-name">${Utils.escapeHtml(chiot.nom)} ${sexSymbol}</div>
            <div class="card-meta" style="margin-top:4px;">${statusBadge}</div>
            ${chiot.couleurRobe ? `<div class="card-meta">🎨 ${Utils.escapeHtml(chiot.couleurRobe)}</div>` : ''}
            ${chiot.poids ? `<div class="card-meta">⚖️ ${chiot.poids}g à la naissance</div>` : ''}
            ...
            ${chiot.notes ? `<div class="card-meta" style="margin-top:4px;font-style:italic;color:var(--text-muted);">📝 ${Utils.escapeHtml(chiot.notes)}</div>` : ''}
          </div>
          <div style="display:flex;flex-direction:column;gap:4px;">
            <button class="btn btn-sm btn-secondary" data-action="edit-chiot" data-portee-id="${porteeId}" data-chiot-id="${chiot.id}" title="Modifier">✏️</button>
            <button class="btn btn-sm btn-secondary" data-action="export-chiot-pdf" data-chiot-id="${chiot.id}" title="Fiche PDF">📄</button>
            <button class="btn btn-sm btn-secondary" data-nav="cession-form" data-params='{"porteeId":"${porteeId}","chiotId":"${chiot.id}"}' title="Cession">📜</button>
          </div>
        </div>
      </div>
    `;
  ...
  // ---- ÉDITION INLINE D'UN CHIOT ----
  function openChiotEditInline(portee, uid, chiotId, espece) {
    const chiot = (portee.chiots || []).find(c => c.id === chiotId);
    if (!chiot) return;
    const card = document.getElementById(`chiot-card-${chiotId}`);
    if (!card) return;
    ...
    card.innerHTML = `
      <div style="padding:4px 0;">
        <div style="font-size:0.85rem;font-weight:700;margin-bottom:10px;color:var(--accent);">✏️ Modifier ${Utils.escapeHtml(chiot.nom)}</div>
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Nom <span class="required">*</span></label>
            <input class="form-input" type="text" id="ce-nom" value="${Utils.escapeHtml(chiot.nom || '')}">
          </div>
          <div class="form-group">
            <label class="form-label">Sexe</label>
            <select class="form-select" id="ce-sexe">
              <option value="male" ${chiot.sexe === 'male' ? 'selected' : ''}>♂ Mâle</option>
              <option value="femelle" ${chiot.sexe === 'femelle' ? 'selected' : ''}>♀ Femelle</option>
            </select>
          </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input class="form-input" type="text" id="ce-couleur" placeholder="Ex: Noir et blanc" value="${Utils.escapeHtml(chiot.couleurRobe || '')}">
          </div>
          <div class="form-group">
            <label class="form-label">Poids naissance (g)</label>
            <input class="form-input" type="number" id="ce-poids" min="0" placeholder="Ex: 350" value="${chiot.poids || ''}">
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Heure naissance</label>
            <input class="form-input" type="time" id="ce-heure" value="${chiot.heure || ''}">
          </div>
          <div class="form-group">
            <label class="form-label">Statut</label>
            <select class="form-select" id="ce-statut">
              <option value="elevage" ${chiot.statut === 'elevage' ? 'selected' : ''}>🏠 En élevage</option>
              <option value="reserve" ${chiot.statut === 'reserve' ? 'selected' : ''}>⭐ Réservé</option>
              <option value="vendu" ${chiot.statut === 'vendu' ? 'selected' : ''}>✅ Vendu</option>
              <option value="cede" ${chiot.statut === 'cede' ? 'selected' : ''}>📤 Cédé</option>
              <option value="decede" ${chiot.statut === 'decede' ? 'selected' : ''}>🕊️ Décédé</option>
            </select>
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Identification (puce/tatouage)</label>
          <input class="form-input" type="text" id="ce-puce" placeholder="N° de puce (15 chiffres) ou tatouage" value="${Utils.escapeHtml(chiot.puce || '')}">
        </div>
        <div class="form-group">
          <label class="form-label">Notes</label>
          <textarea class="form-textarea" id="ce-notes" rows="2" placeholder="Observations, comportement...">${Utils.escapeHtml(chiot.notes || '')}</textarea>
        </div>
        <div style="display:flex;gap:8px;margin-top:10px;">
          <button class="btn btn-primary btn-sm" id="ce-save-btn" style="flex:1;">✅ Enregistrer</button>
          <button class="btn btn-secondary btn-sm" id="ce-cancel-btn">Annuler</button>
        </div>
      </div>
    `;
    ...
    document.getElementById('ce-save-btn').addEventListener('click', async () => {
      ...
      if (!nom) { UI.toast('Le nom est requis', 'error'); return; }

      const updated = {
        ...chiot,
        nom,
        sexe: document.getElementById('ce-sexe').value,
        couleurRobe: document.getElementById('ce-couleur').value.trim(),
        poids: document.getElementById('ce-poids').value.trim(),
        heure: document.getElementById('ce-heure').value,
        puce: document.getElementById('ce-puce').value.trim(),
        statut: document.getElementById('ce-statut').value,
        notes: document.getElementById('ce-notes').value.trim()
      };

      const newChiots = (portee.chiots || []).map(c => c.id === chiotId ? updated : c);

      try {
        await updatePortee(uid, portee.id, { chiots: newChiots });
        portee.chiots = newChiots;
        card.outerHTML = renderChiotDetail(updated, portee.id);
        UI.toast('Chiot mis à jour ✅', 'success');
      } catch (err) {
        console.error('Erreur mise à jour chiot', err);
        UI.toast('Erreur de sauvegarde', 'error');
      }
    });
  }
  ...
  // ---- EXPORT FICHE RÉCAPITULATIF PAR CHIOT ----
  function exportChiotFichePDF(portee, chiot, mereName, pereName, espece) {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
    const W = 190, margin = 10;
    const petit = espece === 'felin' ? 'chaton' : 'chiot';
    ...
    // ---- PAGE 1 : FICHE RÉCAPITULATIF ----
    // Header
    doc.setFillColor(230, 160, 60);
    doc.rect(margin, y - 6, W, 12, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text(`ElevApp — Fiche ${petit}`, margin + 3, y);
    y += 15;
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    const sexSymbol = chiot.sexe === 'male' ? 'Mâle' : 'Femelle';
    doc.text(`${chiot.nom || '—'}`, margin, y);
    y += 6;

    // Sous-titre
    const statutLabels = { elevage: 'En élevage', reserve: 'Réservé', vendu: 'Vendu', cede: 'Cédé', decede: 'Décédé' };
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(120, 120, 120);
    doc.text(`${sexSymbol} — ${statutLabels[chiot.statut] || chiot.statut || '—'}`, margin, y);
    y += 10;
    ...
    // Helper pour un titre de section
    ...
    infoLine('Sexe', sexSymbol);
    infoLine('Couleur / robe', chiot.couleurRobe);
    infoLine('Identification', chiot.puce);
    y += 2;

    // Naissance
    sectionTitle('🍼 Naissance');
    infoLine('Date de naissance', portee.dateNaissance ? Utils.formatDate(portee.dateNaissance) : null);
    infoLine('Poids à la naissance', chiot.poids ? `${chiot.poids} g` : null);
    infoLine('Heure de naissance', chiot.heure);
    y += 2;

    // Parents
    sectionTitle('👨‍👩‍👧 Parents');
    infoLine('Mère', mereName);
    infoLine('Père', pereName);
    y += 2;

    // Portée
    sectionTitle('📊 Portée');
    infoLine('Nés au total', `${portee.nbTotal || 0} ${espece === 'felin' ? 'chatons' : 'chiots'}`);
    infoLine('Vivants', portee.nbVivants || 0);
    const typeMBLabels = { naturelle: 'Naturelle', cesarienne: 'Césarienne', naturelle_cesarienne: 'Naturelle + Césarienne', avortement: 'Avortement', vide: 'Vide', cesarienne_sterilisation: 'Césarienne + Stérilisation' };
    infoLine('Type de mise bas', portee.typeMiseBas ? (typeMBLabels[portee.typeMiseBas] || portee.typeMiseBas) : null);
    y += 2;
    ...
    if (chiot.notes) {
      ...
      const lines = doc.splitTextToSize(chiot.notes, W);
      doc.text(lines, margin, y);
      y += lines.length * 5.5 + 2;
    }

    // ---- PAGE 2 : SUIVI DES POIDS (si disponible) ----
    const suivi = portee.poidsSuivi || [];
    const dates = suivi.map(e => e.date).sort();
    const peseesDuChiot = dates.map(date => {
      const entry = suivi.find(e => e.date === date);
      const pesee = entry?.pesees?.find(p => p.chiotId === chiot.id);
      return pesee ? { date, poids: pesee.poids } : null;
    }).filter(Boolean);

    if (peseesDuChiot.length > 0) {
      doc.addPage();
      y = 15;
      ...
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.text(`Suivi des pesées — ${chiot.nom || '—'}`, margin + 3, y);
      y += 12;
      ...
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      doc.text(`Portée de ${mereName} | Né(e) le ${portee.dateNaissance ? Utils.formatDate(portee.dateNaissance) : '—'} | ${sexSymbol}`, margin, y);
      y += 8;
      ...
      doc.rect(margin, y - 4, W, 7, 'F');
      doc.text('Date', margin + 2, y);
      doc.text('Poids (g)', margin + 40, y);
      doc.text('Évolution', margin + 75, y);
      doc.text('Prise totale', margin + 115, y);
      y += 5;
      doc.setLineWidth(0.3);
      doc.setDrawColor(200, 200, 200);
      doc.line(margin, y - 1, margin + W, y - 1);
      ...
      if (poidsNaissance) {
        doc.setTextColor(120, 120, 120);
        doc.text('Naissance', margin + 2, y + 4);
        doc.text(`${poidsNaissance} g`, margin + 40, y + 4);
        doc.text('—', margin + 75, y + 4);
        doc.text('—', margin + 115, y + 4);
        y += 7;
        doc.line(margin, y - 1, margin + W, y - 1);
      }

      doc.setTextColor(40, 40, 40);
      peseesDuChiot.forEach((pesee, i) => {
        const prevPoids = i > 0 ? peseesDuChiot[i - 1].poids : poidsNaissance;
        const diff = prevPoids !== null ? pesee.poids - prevPoids : null;
        const totalDiff = poidsNaissance ? pesee.poids - poidsNaissance : null;
        const dateStr = new Date(pesee.date + 'T12:00:00').toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: '2-digit' });

        doc.text(dateStr, margin + 2, y + 4);
        doc.text(`${pesee.poids} g`, margin + 40, y + 4);

        if (diff !== null) {
          doc.setTextColor(diff >= 0 ? 0 : 200, diff >= 0 ? 150 : 0, 0);
          doc.text(`${diff >= 0 ? '+' : ''}${diff} g`, margin + 75, y + 4);
          doc.setTextColor(40, 40, 40);
        } else {
          doc.text('—', margin + 75, y + 4);
        }
        if (totalDiff !== null) {
          doc.setTextColor(totalDiff >= 0 ? 0 : 200, totalDiff >= 0 ? 130 : 0, 0);
          doc.text(`${totalDiff >= 0 ? '+' : ''}${totalDiff} g`, margin + 115, y + 4);
          doc.setTextColor(40, 40, 40);
        } else {
          doc.text('—', margin + 115, y + 4);
        }
      ...
      const points = [];
      if (poidsNaissance) points.push({ label: 'Nais.', poids: poidsNaissance });
      peseesDuChiot.forEach(p => {
        const dateStr = new Date(p.date + 'T12:00:00').toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' });
        points.push({ label: dateStr, poids: p.poids });
      });

      if (points.length >= 2 && y < 245) {
        doc.setFontSize(9);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(40, 40, 40);
        doc.text('Courbe de poids', margin, y);
    ...
    for (let i = 1; i <= nbPages; i++) {
      ...
      doc.setTextColor(180, 180, 180);
      doc.text(`Généré par ElevApp — ${new Date().toLocaleDateString('fr-FR')}`, margin, 290);
      if (nbPages > 1) doc.text(`Page ${i}/${nbPages}`, margin + W, 290, { align: 'right' });
    }

    const nomFichier = `fiche_${(chiot.nom || 'chiot').replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
    try {
      const blob = doc.output('blob');
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      ...
      UI.toast('Fiche exportée ! 📄', 'success');
    } catch (err) {
      console.error('Erreur export fiche PDF', err);
