...
const Dashboard = (() => {
  ...
  async function render() {
    UI.setContent(`
      <div class="section-title"><div class="section-icon">${Icons.get('home', 18)}</div> Tableau de bord</div>
    ...
    try {
      // Tout en parallèle en une seule vague
      const [animals, reminders, profile, snapshots, allEntries, dernierControle] = await Promise.all([
        DB.getAnimals(uid),
        DB.getUpcomingReminders(uid, 30),
        DB.getUserProfile(uid),
        DB.getSnapshots(uid),
        DB.getAllHealthEntries(uid),
        Autocontrole.getDernierControle(uid).catch(() => null)
      ]);
      ...
      if (!dernierControle) {
        alerts.push({
          icon: 'clipboardCheck', type: 'orange',
      ...
      let conformity;
      if (hasRed) conformity = { class: 'conformity-red', dot: 'var(--red)', label: 'Obligations non remplies' };
      else if (hasOrange) conformity = { class: 'conformity-orange', dot: 'var(--orange)', label: 'Alertes en attente' };
      else conformity = { class: 'conformity-green', dot: 'var(--green)', label: 'Tout est conforme' };
      ...
      dashEl.innerHTML = `
        ...
        <div class="conformity-indicator ${conformity.class} mb-2">
          <svg width="10" height="10" style="flex-shrink:0;"><circle cx="5" cy="5" r="5" fill="${conformity.dot}"/></svg>
          ${conformity.label}
        </div>
        ...
        <!-- Alertes -->
        ${alerts.length > 0 ? `
          <div class="section-title"><div class="section-icon">${Icons.get('alertTriangle', 18)}</div> Alertes</div>
          <div class="mb-2">
            ${alerts.map(alert => `
              <div class="alert-card mb-1" style="border-left: 3px solid var(--${alert.type === 'red' ? 'red' : 'orange'});">
                <div class="alert-icon">${Icons.get(alert.icon)}</div>
        ...
        <!-- Prochaines échéances -->
        ${echeances.length > 0 ? `
          <div class="section-title"><div class="section-icon">${Icons.get('calendar', 18)}</div> Prochaines échéances</div>
          <div class="mb-2">
            ${echeances.slice(0, 5).map(ech => `
              <div class="alert-card mb-1" style="cursor:pointer;" data-nav="animal-detail" data-params='{"id":"${ech.animalId}"}'>
                <div class="alert-icon">${Icons.get('bell')}</div>
                <div class="alert-content">
                  <div class="alert-title">${Utils.escapeHtml(ech.label)}</div>
                  <div class="alert-desc">${Utils.escapeHtml(ech.animal)} — ${Utils.formatDate(ech.date)}</div>
                </div>
              </div>
            `).join('')}
          </div>
        ...
        <!-- Raccourcis -->
        <div class="section-title"><div class="section-icon">${Icons.get('grid', 18)}</div> Raccourcis</div>
        <div class="shortcut-grid mb-3">
          <button class="shortcut-btn" data-nav="animal-form">
            <div class="shortcut-icon">${Icons.get('paw', 20)}</div>
            + Animal
          </button>
          <button class="shortcut-btn" data-nav="health-form">
            <div class="shortcut-icon">${Icons.get('health', 20)}</div>
            + Entrée
          </button>
          <button class="shortcut-btn" data-nav="exports">
            <div class="shortcut-icon">${Icons.get('fileDown', 20)}</div>
            Exporter PDF
          </button>
          <button class="shortcut-btn" data-nav="registre">
            <div class="shortcut-icon">${Icons.get('bookOpen', 20)}</div>
            Registre E/S
          </button>
          <button class="shortcut-btn" data-nav="autocontrole">
            <div class="shortcut-icon">${Icons.get('clipboardCheck', 20)}</div>
            Autocontrôle
          </button>
          <button class="shortcut-btn" data-nav="portees">
            <div class="shortcut-icon">${Icons.get('litter', 20)}</div>
            Portées
          </button>
          <button class="shortcut-btn" data-nav="calendrier">
            <div class="shortcut-icon">${Icons.get('calendar', 20)}</div>
            Calendrier
          </button>
        </div>
      `;
