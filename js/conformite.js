...
const Conformite = (() => {
  ...
  async function render() {
    UI.setContent(`
      <div class="section-title"><div class="section-icon">${Icons.get('dna', 18)}</div> Conformité Reproduction</div>
    ...
    try {
      ...
      if (femellesRepro.length === 0) {
        globalClass = 'conformity-grey'; globalIcon = '⚪'; globalLabel = 'Aucune femelle reproductrice — ajoutez des animaux pour voir la conformité';
      } else if (alertesRouges > 0) {
      ...
      document.getElementById('conformite-content').innerHTML = `
        ...
        <div class="conformity-indicator ${globalClass} mb-2">
          <svg width="16" height="16" style="display:inline;vertical-align:middle;margin-right:6px;"><circle cx="8" cy="8" r="8" fill="${globalClass === 'conformity-red' ? 'var(--red)' : globalClass === 'conformity-orange' ? 'var(--orange)' : globalClass === 'conformity-green' ? 'var(--green)' : 'var(--text-muted)'}"/></svg>
          ${globalLabel}
        </div>
        ...
        <div class="shortcut-grid mb-2">
          <button class="shortcut-btn" data-nav="health-form" data-params='{"type":"mise_bas"}'>
            <div class="shortcut-icon">${Icons.get('litter', 20)}</div>
            Mise bas
          </button>
          <button class="shortcut-btn" data-nav="health-form" data-params='{"type":"examen_pre_repro"}'>
            <div class="shortcut-icon">${Icons.get('eye', 20)}</div>
            Examen pré-repro
          </button>
          <button class="shortcut-btn" data-nav="health-form" data-params='{"type":"chirurgie"}'>
            <div class="shortcut-icon">${Icons.get('scissors', 20)}</div>
            Césarienne
          </button>
          <button class="shortcut-btn" data-nav="portee-form">
            <div class="shortcut-icon">${Icons.get('penLine', 20)}</div>
            Nouvelle portée
          </button>
        </div>
        ...
        ${femellesRepro.length === 0 ? `
          <div class="empty-state">
            <div class="empty-icon">${Icons.get('dna', 36)}</div>
        ...
        <!-- Outil consanguinité -->
        <div class="section-title mt-3"><div class="section-icon">${Icons.get('dna', 18)}</div> Vérification consanguinité</div>
          ...
          <div class="form-row">
            <div class="form-group" style="flex:1;">
              ...
              <select class="form-select" id="conso-femelle">
                ...
                ${femellesRepro.map(f => `<option value="${f.id}">${Utils.escapeHtml(f.nom)}</option>`).join('')}
              </select>
            </div>
            <div class="form-group" style="flex:1;">
              ...
              <select class="form-select" id="conso-male">
                <option value="">Choisir...</option>
                ${males.map(m => `<option value="${m.id}">${Utils.escapeHtml(m.nom)}</option>`).join('')}
              </select>
            </div>
          </div>
      ...
      document.getElementById('conso-check-btn').addEventListener('click', () => {
        ...
        if (femelle.espece !== male.espece) {
          resultEl.innerHTML = `<div style="padding:10px;border-radius:var(--radius-md);background:var(--red-light);color:var(--red);font-size:0.85rem;">${Icons.get('alertTriangle', 14)} Espèces différentes — accouplement impossible</div>`;
          return;
        }
        ...
        if (issue) {
          resultEl.innerHTML = `<div style="padding:10px;border-radius:var(--radius-md);background:var(--red-light);border-left:3px solid var(--red);font-size:0.85rem;">
            <div style="font-weight:700;color:var(--red);">${Icons.get('x', 14)} Consanguinité interdite</div>
          ...
          if (reproAlertes.some(a => a.type === 'red')) {
            resultEl.innerHTML = `<div style="padding:10px;border-radius:var(--radius-md);background:var(--red-light);border-left:3px solid var(--red);font-size:0.85rem;">
              <div style="font-weight:700;color:var(--red);">${Icons.get('x', 14)} Reproduction interdite pour ${Utils.escapeHtml(femelle.nom)}</div>
            ...
            resultEl.innerHTML = `<div style="padding:10px;border-radius:var(--radius-md);background:var(--orange-light);border-left:3px solid var(--orange);font-size:0.85rem;">
              <div style="font-weight:700;color:var(--orange);">${Icons.get('alertTriangle', 14)} Compatible mais attention</div>
            ...
            resultEl.innerHTML = `<div style="padding:10px;border-radius:var(--radius-md);background:var(--green-light);border-left:3px solid var(--green);font-size:0.85rem;">
              <div style="font-weight:700;color:var(--green);">${Icons.get('checkCircle', 14)} Accouplement autorisé</div>
  ...
  function renderFemelleCard(d) {
    ...
    return `
      <div class="card mb-1" style="cursor:pointer;" data-nav="animal-detail" data-params='{"id":"${animal.id}"}'>
        ...
        ${age >= limiteAge ? `
          <div style="margin-top:8px;padding:6px 10px;border-radius:var(--radius-sm);background:${hasExamen ? 'var(--green-light)' : 'var(--red-light)'};font-size:0.78rem;">
            ${hasExamen
              ? `${Icons.get('checkCircle', 14)} Examen pré-reproduction valide (< 12 mois)`
              : `${Icons.get('alertTriangle', 14)} Examen pré-reproduction requis (${age} ans ≥ ${limiteAge})`
            }
          </div>
        ` : ''}
