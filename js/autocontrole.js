  ...
  async function render() {
    ...
    try {
      ...
      document.getElementById('autocontrole-content').innerHTML = `
        ...
        <div class="section-title"><span class="section-icon">📋</span> Historique</div>
        ...
        ` : `
          ${controles.map(c => {
            ...
            return `
              <div class="card mb-1" onclick="Autocontrole.renderDetail('${c.id}')" style="cursor:pointer;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                  <div>
                    <div style="font-weight:600;">Contrôle du ${Utils.formatDate(c.date)}</div>
                    <div class="card-subtitle">
                      ✅ ${nbConformes} conformes — ❌ ${nbNonConformes} non conformes
                    </div>
      ...
      <form id="controle-form">
        <div class="card mb-2">
          <div class="form-group">
            ...
            <input class="form-input" type="date" id="ctrl-date" required value="${new Date().toISOString().split('T')[0]}">
          </div>
          <div class="form-group">
        ...
        ${Object.entries(categories).map(([cat, items]) => `
          <div class="section-title"><span class="section-icon">${categorieIcons[cat] || '📋'}</span> ${cat}</div>
          <div class="card mb-2">
            ${items.map(item => `
              <div class="autocontrole-item" style="padding:8px 0;border-bottom:1px solid var(--border);">
                ...
                <div style="display:flex;gap:8px;">
                  <label class="form-toggle" style="flex:1;justify-content:center;">
                    ...
                    <span style="font-size:0.8rem;">✅ OK</span>
                  </label>
                  <label class="form-toggle" style="flex:1;justify-content:center;">
                    <input type="radio" name="ctrl-${item.id}" value="non_conforme">
                    <span style="font-size:0.8rem;">❌ NC</span>
                  </label>
                  <label class="form-toggle" style="flex:1;justify-content:center;">
  ...
  async function renderDetail(controleId) {
    ...
    try {
      ...
      document.getElementById('detail-content').innerHTML = `
        ...
        ${Object.entries(categories).map(([cat, items]) => `
          <div class="section-title"><span class="section-icon">📋</span> ${cat}</div>
          <div class="card mb-2">
            ${items.map(item => {
