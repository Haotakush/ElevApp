...
const Chaleurs = (() => {
      ...
      <div style="margin-top:8px;border-top:1px solid var(--border);padding-top:8px;">
        ...
        <div style="margin-top:6px;">
          ${progesLines.map(p => {
            ...
            const color = valF >= 20 ? 'var(--green)' : valF >= 5 ? 'var(--orange)' : 'var(--text-primary)';
            return `<div style="font-size:0.75rem;padding:2px 0;display:flex;gap:6px;align-items:center;">
              ...
              ${p.notes ? `<span style="color:var(--text-muted);font-style:italic;">${Utils.escapeHtml(p.notes)}</span>` : ''}
            </div>`;
          }).join('')}
        </div>
      </div>
        ...
        <form id="chaleur-form">
          ...
          <div class="card mb-2">
            <div class="form-group">
              ...
              <input type="date" class="form-control" id="cf-date" value="${toInputDate(c.date)}" required>
            </div>
            <div class="form-group">
          ...
          <div class="card mb-2">
            ...
            <div class="form-group">
              ...
              <input type="date" class="form-control" id="cf-saillie-date" value="${toInputDate(saillie.date)}">
            </div>
            <div class="form-group">
              <label class="form-label">Date saillie (2ème — optionnel)</label>
              <input type="date" class="form-control" id="cf-saillie-date2" value="${toInputDate(saillie.date2)}">
            </div>
            <div class="form-group">
            ...
            <div id="mise-bas-section" style="display:${c.resultat === 'mise_bas' ? 'block' : 'none'};">
              <div style="border-top:1px solid var(--border);padding-top:12px;margin-top:8px;">
                <div class="form-group">
                  ...
                  <input type="date" class="form-control" id="cf-mb-date" value="${toInputDate(c.dateMiseBas)}">
                </div>
                ...
                <div id="mammite-notes-group" style="display:${c.mammite ? 'block' : 'none'};margin-top:10px;padding:10px;border-radius:var(--radius-md);background:rgba(239,68,68,0.06);border:1px solid rgba(239,68,68,0.2);">
                  ...
                  <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                    <div class="form-group">
                      ...
                      <input type="date" class="form-control" id="cf-mammite-debut" value="${toInputDate(c.mammiteDateDebut)}">
                    </div>
                    <div class="form-group">
                      <label class="form-label">Date fin / guérison</label>
                      <input type="date" class="form-control" id="cf-mammite-fin" value="${toInputDate(c.mammiteDateFin)}">
                    </div>
                  </div>
                  <div class="form-group">
              ...
              <select class="form-control" id="cf-portee-id">
                ...
                ${portees.map(p => {
                  ...
                  return `<option value="${p.id}" ${(existingChaleur?.porteeId === p.id) ? 'selected' : ''}>
                    Portée ${dateStr} — ${p.nbTotal || '?'} chiot${p.nbTotal > 1 ? 's' : ''}
                  </option>`;
                }).join('')}
              </select>
  ...
  function renderProgesList() {
      ...
      <div class="proges-row" data-index="${i}" style="border:1px solid var(--border);border-radius:var(--radius-md);padding:10px;margin-bottom:8px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
          <div class="form-group" style="margin:0;">
            ...
            <input type="date" class="form-control proges-date" data-index="${i}" value="${p.date || ''}">
          </div>
          <div class="form-group" style="margin:0;">
        ...
        <div style="display:grid;grid-template-columns:1fr 2fr;gap:8px;margin-top:8px;">
          ...
          <div class="form-group" style="margin:0;">
            ...
            <input type="text" class="form-control proges-notes" data-index="${i}" placeholder="Vétérinaire, labo..." value="${Utils.escapeHtml(p.notes || '')}">
          </div>
        </div>
      </div>
    `).join('');
    ...
    container.querySelectorAll('.proges-valeur').forEach(inp => {
      inp.addEventListener('input', () => { progesLineItems[parseInt(inp.dataset.index)].valeur = inp.value; });
    });
    ...
    try {
      ...
      const data = {
        ...
        saillie: {
          ...
          date: document.getElementById('cf-saillie-date').value,
          date2: document.getElementById('cf-saillie-date2').value,
          pereNom: pereNomVal,
          pereId: pereIdVal
        },
        resultat,
      ...
      if (resultat === 'mise_bas') {
        data.dateMiseBas = document.getElementById('cf-mb-date').value;
        ...
        if (mammite) {
          ...
          data.mammiteDateDebut = document.getElementById('cf-mammite-debut')?.value || '';
          data.mammiteDateFin = document.getElementById('cf-mammite-fin')?.value || '';
  ...
  async function exportPDFChaleurs(uid, animalId, animal, chaleurs, numMap) {
    ...
    try {
      ...
      doc.save(`elevapp-suivi-repro-${(animal?.nom || 'animal').toLowerCase()}-${dateStr}.pdf`);
