...
const Health = (() => {
  ...
  async function renderJournal() {
    UI.setContent(`
      <div class="section-title"><div class="section-icon">${Icons.get('health', 18)}</div> Journal sanitaire</div>
      ...
      <div class="filters-bar" id="health-filters">
        ...
        ${Object.entries(Utils.TYPES_SANTE).map(([key, info]) =>
          `<button class="filter-chip ${currentFilter === key ? 'active' : ''}" data-value="${key}">${Icons.get(info.icon, 12)} ${info.label}</button>`
        ).join('')}
      </div>
      ...
      if (entries.length === 0) {
        listEl.innerHTML = `
          <div class="empty-state">
            <div class="empty-icon">${Icons.get('health', 36)}</div>
            ...
            <div class="timeline-item ${isAlert ? 'alert' : ''}">
              <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
                <div style="flex:1;cursor:pointer;" data-nav="animal-detail" data-params='{"id":"${entry.animalId}"}'>
                  ...
                  ${isAlert ? `<div class="timeline-details" style="display:flex;align-items:center;gap:4px;color:var(--red);">${Icons.get('bell', 12)} Rappel dépassé : ${Utils.formatDate(entry.rappelDate)}</div>` : ''}
                </div>
  ...
  async function renderForm(preAnimalId, entryId, preType) {
      ...
      <form id="health-form">
        ...
        <div class="form-group">
          ...
          <select class="form-select" id="health-animal" required ${preAnimalId ? 'disabled' : ''}>
            ...
            ${activeAnimals.map(a =>
              `<option value="${a.id}" ${a.id === selectedAnimal ? 'selected' : ''}>${Utils.escapeHtml(a.nom)}</option>`
            ).join('')}
          </select>
        ...
        <div class="form-group">
          ...
          <select class="form-select" id="health-type" required>
            ...
            ${Object.entries(Utils.TYPES_SANTE).map(([key, info]) =>
              `<option value="${key}" ${key === selectedType ? 'selected' : ''}>${info.label}</option>`
            ).join('')}
          </select>
        </div>
    ...
    typeSelect.addEventListener('change', () => {
      const curAnimalId = document.getElementById('health-animal-hidden')?.value || document.getElementById('health-animal')?.value;
      const curAnimalObj = animals.find(a => a.id === curAnimalId) || selectedAnimalObj;
      updateConditionalFields(curAnimalObj, animals);
      updateLabelsForType();
    });

    // Mettre à jour les champs conditionnels quand l'animal change aussi
    const animalSelect = document.getElementById('health-animal');
    if (animalSelect && !animalSelect.disabled) {
      animalSelect.addEventListener('change', () => {
        const curAnimalId = animalSelect.value;
        const curAnimalObj = animals.find(a => a.id === curAnimalId);
        updateConditionalFields(curAnimalObj, animals);
      });
    }

    updateConditionalFields(selectedAnimalObj, animals);
    updateLabelsForType();
  ...
  function updateConditionalFields(animalObj, allAnimals) {
    const type = document.getElementById('health-type').value;
    const container = document.getElementById('health-conditional-fields');
    const selectedAnimalId = document.getElementById('health-animal-hidden')?.value || document.getElementById('health-animal')?.value;
    const espece = animalObj?.espece || (allAnimals || []).find(a => a.id === selectedAnimalId)?.espece || 'canin';
