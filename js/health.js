...
const Health = (() => {
    ...
    try {
      ...
      if (entries.length === 0) {
        listEl.innerHTML = `
          <div class="empty-state">
            ...
            <button class="btn btn-primary" onclick="UI.navigateTo('health-form')">+ Nouvelle entrée</button>
          </div>
        `;
        return;
      }
      ...
      listEl.innerHTML = `<div class="timeline">
        ${entries.map(entry => {
          ...
          return `
            <div class="timeline-item ${isAlert ? 'alert' : ''}" style="cursor:pointer;" onclick="UI.navigateTo('animal-detail', {id:'${entry.animalId}'})">
  ...
  async function renderForm(preAnimalId, entryId, preType) {
    ...
    UI.setContent(`
      ${UI.pageHeader(isEdit ? 'Modifier l\'entrée' : 'Nouvelle entrée sanitaire', preAnimalId ? 'animal-detail' : 'health')}
      <div class="skeleton skeleton-card"></div>
    `);
    ...
    UI.setContent(`
      ...
      <form id="health-form">
        ...
        <div class="form-group">
          ...
          <input class="form-input" type="date" id="health-date" required value="${entry.date ? Utils.formatDateISO(entry.date) : new Date().toISOString().split('T')[0]}">
        </div>

        <!-- Titre -->
        <div class="form-group">
          <label class="form-label" for="health-titre">Titre / Résumé court <span class="required">*</span></label>
        ...
        <!-- Détails -->
        <div class="form-group">
          <label class="form-label" for="health-details">Détails</label>
        ...
        <div class="form-group" id="health-rappel-group">
          ...
          <input class="form-input" type="date" id="health-rappel" value="${entry.rappelDate ? Utils.formatDateISO(entry.rappelDate) : ''}">
    ...
    typeSelect.addEventListener('change', () => updateConditionalFields(selectedAnimalObj));
    ...
    document.getElementById('health-form').addEventListener('submit', async (e) => {
      ...
      const data = {
        ...
        rappelDate: rappelVal ? firebase.firestore.Timestamp.fromDate(new Date(rappelVal)) : null,
        documentsURLs: [],
          ...
          if (rappelMois) {
            rappelMois.addEventListener('change', () => {
              ...
              if (dateVal && rappelMois.value) {
                ...
                document.getElementById('health-rappel').value = Utils.formatDateISO(rappelDate);
              }
            });
            // Trigger initial
  ...
  return { renderJournal, renderForm };
})();
