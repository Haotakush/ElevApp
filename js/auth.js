...
const Auth = (() => {
  ...
  let especeElevee = 'les_deux'; // Par défaut : affiche tout

  function getUser() { return currentUser; }
  function getUid() { return currentUser ? currentUser.uid : null; }
  function getEspeceElevee() { return especeElevee; }
  function getPetit() { return especeElevee === 'felin' ? 'chaton' : 'chiot'; }
  function getPetits() { return especeElevee === 'felin' ? 'chatons' : 'chiots'; }
          ...
          <div class="form-group" style="margin-top:12px;">
            <label class="form-toggle">
              ...
              <span style="font-size:0.82rem;line-height:1.4;">
                J'accepte les <a href="mentions-legales.html" target="_blank" style="color:var(--primary);text-decoration:underline;">CGU</a>
                et la <a href="#" id="rgpd-link" style="color:var(--primary);text-decoration:underline;">politique de confidentialité</a>
  ...
  async function renderProfile() {
    ...
    UI.setContent(`
      ...
      <form id="profile-form">
        ...
        <div class="card mb-2">
          <div class="form-group">
            <label class="form-label" for="prof-espece">Espèce élevée <span class="required">*</span></label>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:4px;">
              <label style="display:flex;align-items:center;gap:10px;padding:12px;border:2px solid ${profile.especeElevee === 'canin' ? 'var(--accent)' : 'var(--border)'};border-radius:var(--radius-md);cursor:pointer;background:${profile.especeElevee === 'canin' ? 'var(--accent-light)' : 'transparent'};" id="espece-canin-label">
                <input type="radio" name="prof-espece" id="prof-espece-canin" value="canin" ${profile.especeElevee === 'canin' ? 'checked' : ''} style="display:none;">
                <span style="font-size:2rem;">🐕</span>
                <span style="font-weight:600;">Canin</span>
              </label>
              <label style="display:flex;align-items:center;gap:10px;padding:12px;border:2px solid ${profile.especeElevee === 'felin' ? 'var(--accent)' : 'var(--border)'};border-radius:var(--radius-md);cursor:pointer;background:${profile.especeElevee === 'felin' ? 'var(--accent-light)' : 'transparent'};" id="espece-felin-label">
                <input type="radio" name="prof-espece" id="prof-espece-felin" value="felin" ${profile.especeElevee === 'felin' ? 'checked' : ''} style="display:none;">
                <span style="font-size:2rem;">🐈</span>
                <span style="font-weight:600;">Félin</span>
              </label>
            </div>
            <input type="hidden" id="prof-espece" value="${profile.especeElevee || ''}">
            <div class="form-hint">Ce choix adapte toute l'application à votre élevage.</div>
          </div>
        </div>
    ...
    // ---- Sélecteur espèce visuel ----
    ['canin', 'felin'].forEach(esp => {
      const label = document.getElementById(`espece-${esp}-label`);
      const radio = document.getElementById(`prof-espece-${esp}`);
      const hidden = document.getElementById('prof-espece');
      if (!label) return;
      label.addEventListener('click', () => {
        radio.checked = true;
        hidden.value = esp;
        ['canin', 'felin'].forEach(e => {
          const l = document.getElementById(`espece-${e}-label`);
          if (l) {
            l.style.borderColor = e === esp ? 'var(--accent)' : 'var(--border)';
            l.style.background = e === esp ? 'var(--accent-light)' : 'transparent';
          }
        });
      });
    });
    ...
    document.getElementById('profile-form').addEventListener('submit', async (e) => {
      ...
      try {
        const newEspece = document.getElementById('prof-espece').value;
  ...
  async function onAuthStateChanged(user) {
    currentUser = user;
    if (user) {
      // Créer le profil si première connexion — retourne true si nouvel utilisateur
      const isNew = await ensureProfile(user);
      UI.showAppShell();
      UI.updateProfileButton(user);

      // Consentement RGPD obligatoire pour les nouveaux utilisateurs Google SSO
      if (isNew) {
        showFirstTimeConsentOverlay(user);
        return;
      }
      ...
      setTimeout(() => {
        if (typeof Onboarding !== 'undefined') {
          const obKey = `elevapp_onboarding_done_${user.uid}`;
          if (!localStorage.getItem(obKey)) {
            Onboarding.checkAndShow(user.uid);
          } else {
  ...
  // ---- Overlay consentement premier login (Google SSO) ----
  function showFirstTimeConsentOverlay(user) {
    // Créer un overlay plein écran bloquant l'accès à l'app
    const overlay = document.createElement('div');
    overlay.id = 'consent-overlay';
    overlay.style.cssText = `
      position:fixed;top:0;left:0;width:100%;height:100%;
      background:var(--bg,#f9f9f7);z-index:9999;
      display:flex;align-items:center;justify-content:center;
      padding:24px;overflow-y:auto;
    `;
    overlay.innerHTML = `
      <div style="max-width:480px;width:100%;background:var(--surface,white);border-radius:16px;padding:28px 24px;box-shadow:0 4px 24px rgba(0,0,0,0.12);">
        <div style="font-size:2rem;text-align:center;margin-bottom:8px;">🐾</div>
        <h2 style="font-size:1.15rem;font-weight:700;text-align:center;margin-bottom:4px;">Bienvenue sur ElevApp</h2>
        <p style="font-size:0.85rem;color:var(--text-muted,#666);text-align:center;margin-bottom:20px;">
          Avant d'accéder à l'application, merci d'accepter nos conditions.
        </p>

        <div style="background:var(--bg,#f9f9f7);border-radius:10px;padding:16px;margin-bottom:20px;font-size:0.82rem;line-height:1.6;color:var(--text-muted,#555);">
          <p style="margin:0 0 10px 0;">ElevApp collecte et traite vos données personnelles (email, données d'élevage) pour le fonctionnement du service, conformément au <strong>RGPD</strong>.</p>
          <p style="margin:0 0 10px 0;">
            📄 <a href="mentions-legales.html" target="_blank" style="color:var(--primary,#E8983E);">Lire les CGU & Mentions légales</a><br>
            🔒 <a href="confidentialite.html" target="_blank" style="color:var(--primary,#E8983E);">Lire la Politique de confidentialité</a>
          </p>
          <p style="margin:0;">Vous pouvez supprimer votre compte et toutes vos données à tout moment depuis Paramètres → Supprimer mon compte.</p>
        </div>

        <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;margin-bottom:20px;">
          <input type="checkbox" id="consent-checkbox" style="margin-top:3px;flex-shrink:0;accent-color:var(--primary,#E8983E);">
          <span style="font-size:0.83rem;line-height:1.5;">
            J'ai lu et j'accepte les <a href="mentions-legales.html" target="_blank" style="color:var(--primary,#E8983E);">CGU</a>
            et la <a href="confidentialite.html" target="_blank" style="color:var(--primary,#E8983E);">Politique de confidentialité</a>.
            J'accepte le traitement de mes données conformément au RGPD. <span style="color:var(--danger,#e53e3e);">*</span>
          </span>
        </label>
        <div id="consent-error" style="color:var(--danger,#e53e3e);font-size:0.82rem;margin-bottom:12px;display:none;">
          Vous devez accepter les conditions pour utiliser ElevApp.
        </div>
        <button id="consent-accept-btn" class="btn btn-primary btn-block">
          Accéder à l'application →
        </button>
        <button id="consent-refuse-btn" class="btn btn-block" style="margin-top:8px;background:transparent;color:var(--text-muted,#666);border:none;font-size:0.82rem;">
    ...
    document.getElementById('consent-accept-btn').addEventListener('click', async () => {
      ...
      try {
        ...
        console.error('Erreur save consent', e);
      }
      overlay.remove();
      UI.navigateTo('dashboard');
      setTimeout(() => {
    ...
    document.getElementById('consent-refuse-btn').addEventListener('click', async () => {
      await firebase.auth().signOut();
      overlay.remove();
    });
  }
  ...
  async function ensureProfile(user) {
    ...
    // Charger l'espèce élevée globalement
    if (profile?.especeElevee) especeElevee = profile.especeElevee;
    if (!profile) {
      // Nouveau profil (Google SSO ou inscription sans profil Firestore)
      await DB.setUserProfile(user.uid, {
        ...
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
        dernierSnapshot: null
        // rgpdConsentDate sera ajouté après l'overlay de consentement
      });
  ...
  return {
    getUser, getUid, getEspeceElevee, getPetit, getPetits,
