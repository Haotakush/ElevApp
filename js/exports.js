...
const Exports = (() => {
  ...
  async function render() {
    ...
    UI.setContent(`
      <div class="section-title"><span class="section-icon">📊</span> Exports & Archivage</div>
      ...
      <div class="card mb-2">
        ...
        <button class="btn btn-primary btn-block" onclick="UI.navigateTo('cession-form')">
      ...
      <div class="card mb-2">
        ...
        <button class="btn btn-primary btn-block" onclick="UI.navigateTo('registre')">
          📒 Ouvrir le registre
        </button>
      </div>
      ...
      <div class="card mb-2">
        ...
        <button class="btn btn-primary btn-block" onclick="UI.navigateTo('autocontrole')">
      ...
      <div class="section-title mt-3"><span class="section-icon">📚</span> Historique des snapshots</div>
    ...
    try {
      ...
      listEl.innerHTML = snapshots.map(snap => `
        <div class="card mb-1">
          ...
          ${snap.pdfURL ? `<a href="${snap.pdfURL}" target="_blank" class="btn btn-secondary btn-sm mt-1">📥 Télécharger</a>` : ''}
        </div>
      `).join('');
    } catch (err) {
    ...
    try {
      ...
      for (const animal of animals) {
        ...
        if (animal.sexe === 'femelle' && animal.statutReproducteur === 'nonSterilise' && animal.statut === 'actif') {
          const entries2 = await DB.getHealthEntries(uid, animal.id);
  ...
  async function generateSnapshot(uid) {
    ...
    try {
      ...
      for (const animal of animals) {
        ...
        if (animal.sexe === 'femelle' && animal.statutReproducteur === 'nonSterilise' && animal.statut === 'actif') {
          const allEntriesAnimal = await DB.getHealthEntries(uid, animal.id);
          const cesariennes = Utils.countCesariennes(allEntriesAnimal);
          const portees2ans = Utils.countPortees2Ans(allEntriesAnimal);
          const reproAlertes = Utils.analyseReproduction(animal, allEntriesAnimal);
