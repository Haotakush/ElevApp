...
const Animals = (() => {
  ...
  async function renderList() {
    UI.setContent(`
      <div class="section-title"><div class="section-icon">${Icons.get('paw', 18)}</div> Mes animaux</div>

      <!-- Recherche -->
      <div class="search-bar">
        <div class="search-icon">${Icons.get('search', 16)}</div>
      ...
      if (animals.length === 0) {
        listEl.innerHTML = `
          <div class="empty-state">
            <div class="empty-icon">${Icons.get('paw', 36)}</div>
            <div class="empty-title">Aucun animal</div>
            <div class="empty-desc">Commencez par ajouter votre premier animal</div>
            <button class="btn btn-primary" data-nav="animal-form">+ Ajouter un animal</button>
          </div>
        `;
        return;
      }
  ...
  async function renderDetail(animalId) {
    ...
    try {
      ...
      let mereNom = '—', pereNom = '—';
      if (mereData) mereNom = `<a href="#" data-nav="animal-detail" data-params='{"id":"${animal.parentMereId}"}'>${Utils.escapeHtml(mereData.nom)}</a>`;
      if (pereData) pereNom = `<a href="#" data-nav="animal-detail" data-params='{"id":"${animal.parentPereId}"}'>${Utils.escapeHtml(pereData.nom)}</a>`;
      ...
      UI.setContent(`
        ...
        <div class="conformity-indicator ${conformity.class} mb-2">
          <svg width="10" height="10" style="flex-shrink:0;"><circle cx="5" cy="5" r="5" fill="${conformity.dot || 'var(--green)'}"/></svg>
          ${conformity.label}
        </div>
        ...
        ${animal.sexe === 'femelle' && animal.statutReproducteur === 'nonSterilise' ? (() => {
          ...
          return `
            <div class="section-title"><div class="section-icon">${Icons.get('litter', 18)}</div> Reproduction (Art. 26)</div>
            <div class="card mb-2">
              <div class="info-row">
              ...
              ${reproAlertes.length > 0 ? reproAlertes.map(ra => `
                <div style="padding:8px 12px;margin-top:8px;border-radius:var(--radius-md);background:var(--bg-secondary);border-left:3px solid var(--${ra.type === 'red' ? 'red' : 'orange'});">
                  <div style="display:flex;align-items:center;gap:6px;font-weight:600;font-size:0.85rem;">${Icons.get(ra.icon, 14)} ${ra.titre}</div>
                  <div style="font-size:0.78rem;color:var(--text-muted);margin-top:2px;">${ra.desc}</div>
                </div>
              `).join('') : '<div style="padding:8px 12px;margin-top:8px;display:flex;align-items:center;gap:6px;color:var(--green);font-size:0.85rem;">' + Icons.get('checkCircle', 14) + ' Apte à la reproduction</div>'}
            </div>
          `;
        })() : ''}
        ...
        ${animal.sexe === 'femelle' && animal.statutReproducteur !== 'sterilise' ? `
          <div class="section-title"><div class="section-icon">${Icons.get('thermometer', 18)}</div> Chaleurs &amp; Cycles</div>
          <div class="card mb-2" style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
            ...
            <button class="btn btn-secondary btn-sm" data-nav="chaleurs-list" data-params='{"animalId":"${animalId}"}'>Voir →</button>
          </div>
        ` : ''}
        ...
        ${(() => {
          ...
          ` : '';
          return `
            <div class="section-title"><div class="section-icon">${Icons.get('scale', 18)}</div> Suivi du poids</div>
            <div class="card mb-2">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                ...
                <button class="btn btn-secondary btn-sm" data-nav="health-form" data-params='{"animalId":"${animalId}","type":"pesee"}'>+ Pesée</button>
              </div>
              ${svgChart}
              ${pesees.length > 1 ? `
        ...
        <!-- Actions rapides -->
        <div class="section-title"><div class="section-icon">${Icons.get('grid', 18)}</div> Actions rapides</div>
        <div class="quick-actions mb-2">
          <button class="quick-action-btn" data-nav="health-form" data-params='{"animalId":"${animalId}","type":"vaccin"}'>${Icons.get('syringe', 14)} Vaccin</button>
          <button class="quick-action-btn" data-nav="health-form" data-params='{"animalId":"${animalId}","type":"traitement"}'>${Icons.get('bug', 14)} Traitement</button>
          <button class="quick-action-btn" data-nav="health-form" data-params='{"animalId":"${animalId}","type":"visite_veto"}'>${Icons.get('heartPulse', 14)} Visite véto</button>
          <button class="quick-action-btn" data-nav="health-form" data-params='{"animalId":"${animalId}","type":"chirurgie"}'>${Icons.get('scissors', 14)} Chirurgie</button>
          <button class="quick-action-btn" data-nav="health-form" data-params='{"animalId":"${animalId}","type":"pesee"}'>${Icons.get('scale', 14)} Pesée</button>
          <button class="quick-action-btn" data-nav="cession-form" data-params='{"animalId":"${animalId}"}'>${Icons.get('penLine', 14)} Cession</button>
          ${animal.sexe === 'femelle' && animal.statutReproducteur !== 'sterilise' ? `<button class="quick-action-btn" data-nav="chaleur-form" data-params='{"animalId":"${animalId}"}'>${Icons.get('thermometer', 14)} Chaleur</button>` : ''}
        </div>

        <!-- Boutons éditer / supprimer -->
        <div style="display:flex;gap:10px;margin-bottom:16px;">
          <button class="btn btn-secondary" style="flex:1;display:flex;align-items:center;justify-content:center;gap:6px;" data-nav="animal-form" data-params='{"id":"${animalId}"}'>${Icons.get('edit', 16)} Modifier</button>
          <button class="btn btn-danger btn-sm" id="delete-animal-btn" style="display:flex;align-items:center;justify-content:center;">${Icons.get('trash', 16)}</button>
        </div>

        <!-- Timeline sanitaire -->
        <div class="section-title"><div class="section-icon">${Icons.get('health', 18)}</div> Historique sanitaire</div>
  ...
  function renderTimelineItem(entry) {
    const typeInfo = Utils.TYPES_SANTE[entry.type] || { label: entry.type, icon: 'penLine' };
      ...
      <div class="timeline-item ${isAlert ? 'alert' : ''}">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
          <div style="flex:1;">
            ...
            <div class="timeline-type" style="display:flex;align-items:center;gap:5px;">${Icons.get(typeInfo.icon, 14)} ${typeInfo.label}</div>
            ...
            ${entry.rappelDate ? `<div class="timeline-details" style="display:flex;align-items:center;gap:4px;color:${isAlert ? 'var(--red)' : 'var(--orange)'};">${Icons.get('bell', 12)} Rappel : ${Utils.formatDate(entry.rappelDate)}${isAlert ? ' (dépassé !)' : ''}</div>` : ''}
          </div>
          <div style="display:flex;gap:4px;flex-shrink:0;margin-top:2px;">
            <button data-nav="health-form" data-params='{"animalId":"${entry.animalId}","entryId":"${entry.id}"}' data-stop="true"
              style="background:none;border:1px solid var(--border);border-radius:6px;padding:4px 8px;cursor:pointer;display:flex;align-items:center;color:var(--text-secondary);">
              ${Icons.get('edit', 14)}
            </button>
            <button data-action="delete-health-entry" data-animal-id="${entry.animalId}" data-entry-id="${entry.id}" data-stop="true"
              style="background:none;border:1px solid var(--border);border-radius:6px;padding:4px 8px;cursor:pointer;display:flex;align-items:center;color:var(--red);">
              ${Icons.get('trash', 14)}
            </button>
          </div>
        </div>
  ...
  function getAnimalConformity(animal, healthEntries) {
    ...
    if (issues.length === 0) return { class: 'conformity-green', dot: 'var(--green)', label: 'Conforme' };
    if (hasRed) return { class: 'conformity-red', dot: 'var(--red)', label: `Non conforme : ${issues.join(', ')}` };
    if (issues.length <= 2) return { class: 'conformity-orange', dot: 'var(--orange)', label: `Attention : ${issues.join(', ')}` };
    return { class: 'conformity-red', dot: 'var(--red)', label: `Non conforme : ${issues.join(', ')}` };
  }
  ...
  async function renderForm(animalId) {
    ...
    UI.setContent(`
      ${UI.pageHeader(isEdit ? 'Modifier l\'animal' : 'Nouvel animal', isEdit ? 'animal-detail' : 'animals', isEdit ? {id: animalId} : {})}
      <div class="skeleton skeleton-card"></div>
    `);
    ...
    UI.setContent(`
      ...
      <form id="animal-form">
          ...
          <div class="photo-upload" id="photo-upload">
            ${animal.photoURL
              ...
              : `<div class="upload-placeholder" id="photo-placeholder">
                  <div class="upload-icon">${Icons.get('camera', 22)}</div>
        ...
        <div class="section-title mt-2"><div class="section-icon">${Icons.get('layers', 18)}</div> Informations complémentaires</div>
        <div class="card mb-2">
          <!-- Potentiel -->
          <div class="form-group">
        ...
        <div class="section-title mt-2"><div class="section-icon">${Icons.get('bookOpen', 18)}</div> Enregistrement & Identification</div>
        <div class="card mb-2">
          <!-- N° LOF/LOOF -->
          <div class="form-group">
        ...
        <div class="form-group">
          ...
          <select class="form-select" id="anim-statutRepro" required>
            <option value="">Choisir...</option>
    ...
    // ---- Statut reproducteur "Autre" toggle ----
    document.getElementById('anim-statutRepro').addEventListener('change', (e) => {
      _toggleStatutAutre(e.target.value);
    });
