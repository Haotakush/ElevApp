...
const Animals = (() => {
  ...
  async function loadAnimalsList() {
    ...
    try {
      let animals = await DB.getAnimals(uid, {
        espece: currentFilters.espece || undefined,
  ...
  async function renderDetail(animalId) {
    ...
    try {
      ...
      const healthEntries = await DB.getHealthEntries(uid, animalId);

      // Récupérer noms des parents
      let mereNom = '—', pereNom = '—';
      if (animal.parentMereId) {
        const mere = await DB.getAnimal(uid, animal.parentMereId);
        if (mere) mereNom = `<a href="#" onclick="UI.navigateTo('animal-detail', {id:'${animal.parentMereId}'}); return false;">${Utils.escapeHtml(mere.nom)}</a>`;
      }
      if (animal.parentPereId) {
        const pere = await DB.getAnimal(uid, animal.parentPereId);
        if (pere) pereNom = `<a href="#" onclick="UI.navigateTo('animal-detail', {id:'${animal.parentPereId}'}); return false;">${Utils.escapeHtml(pere.nom)}</a>`;
      }
      ...
      UI.setContent(`
        ...
        <div class="card mb-2">
          ...
          <div class="info-row">
            ...
            <span class="info-value">${Utils.STATUTS_REPRODUCTEUR[animal.statutReproducteur] || '—'}</span>
          </div>
          <div class="info-row">
  ...
  function getAnimalConformity(animal, healthEntries) {
    // Vérifier : identification, vaccins à jour, visite véto
    const issues = [];
  ...
  async function renderForm(animalId) {
      ...
      <form id="animal-form">
        ...
        <div class="form-group">
          ...
          <select class="form-select" id="anim-espece" required>
            ...
            <option value="canin" ${animal.espece === 'canin' ? 'selected' : ''}>🐕 Canin</option>
            <option value="felin" ${animal.espece === 'felin' ? 'selected' : ''}>🐈 Félin</option>
          </select>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input class="form-input" type="date" id="anim-dateNaissance" required value="${animal.dateNaissance ? Utils.formatDateISO(animal.dateNaissance) : ''}">
          </div>
    ...
    photoInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        upload.innerHTML = `<img src="${url}" alt="Photo" id="photo-preview"><input type="file" accept="image/*" id="photo-input" capture="environment">`;
        // Re-attach listener
    ...
    document.getElementById('animal-form').addEventListener('submit', async (e) => {
      ...
      const data = {
        ...
        dateNaissance: firebase.firestore.Timestamp.fromDate(new Date(document.getElementById('anim-dateNaissance').value)),
